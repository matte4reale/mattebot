...
export default async function (m, conn = { user: {} }) {
  ...
  try {
    // Usa decodeJid in modo unificato per evitare problemi con LID/JID.
    const senderJid = conn.decodeJid(m.sender)
    const chatJid = conn.decodeJid(m.chat || '') // Fallback to empty string if m.chat is null
    ...
    // Fix: Anti-duplicato log per same JID/sender
    ...
    const righe = [
      `${bordi.top}`,
      `${bordi.pipe} ${c.label('ð± Bot:')} ${c.text(me)}`,
      `${bordi.pipe} ${c.label('â° Ora:')} ${c.text(ts)}${messageAge ? c.secondary(' â¢ ') + c.meta(messageAge) : ''}`,
      `${bordi.pipe} ${c.label('ð¤ Da:')} ${c.text(sender)}${isGroup ? ` ${c.secondary('|')} ${getUserStatus(isOwner, isAdmin, isPremium, isBanned, c)}` : ''}`,
      `${bordi.pipe} ${c.label('ð¬ Chat:')} ${c.text(chat)}${isGroup ? c.secondary(' (Gruppo)') : c.secondary(' (Privato)')}`,
      `${bordi.pipe} ${c.label('ð¨ Tipo:')} ${c.text(tipo)}${getMessageFlags(m, c)}`
    ]

    if (filesize) righe.push(`${bordi.pipe} ${c.label('ð¦ Dimensione:')} ${c.text(formatSize(filesize))}`)
    if (m.isCommand) righe.push(`${bordi.pipe} ${c.label('âï¸ Comando:')} ${c.text(getCommand(m.text))}`)
    if (user.exp !== '?') righe.push(`${bordi.pipe} ${c.label('â­ EXP:')} ${c.text(user.exp)}${user.euro !== '?' ? c.secondary(' | ') + c.label('ð° ') + c.text(user.euro) : ''}`)
    if (isGroup && chatJid) {
      try {
        const groupMeta = await conn.groupMetadata(chatJid)
        const participantCount = groupMeta?.participants?.length || '?'
        righe.push(`${bordi.pipe} ${c.label('ð¥ Membri:')} ${c.text(participantCount)}`)
      } catch (e) {
    ...
    if (m.broadcast) {
      righe.push(`${bordi.pipe} ${c.label('ð¢ Broadcast:')} ${c.text('SÃ¬')}`)
    }
    ...
    if (m.reactions && m.reactions.length > 0) {
      ...
      console.log(`${c.secondary('ð­ Reazioni:')} ${c.text(reactions)}`)
    }

    if (m.editedTimestamp) {
      const editTime = new Date(m.editedTimestamp * 1000).toLocaleTimeString('it-IT')
      console.log(`${c.secondary('âï¸ Modificato:')} ${c.text(editTime)}`)
    }
...
function getBorders(c) {
  return {
    top: `${c.secondary.bold('âââââââââââââ')}ã ${c.success.bold('VARE â§ BOT')} ã${c.secondary.bold('âââââââââââââ')}`,
    bottom: `${c.secondary.bold('âââââââââââââââââââââââââââââââââââââââââ')}${c.secondary.bold('â')}`,
...
function logMessageSpecifics(m, c) {
  const specifics = {
    documentMessage: () => {
      ...
      console.log(`${c.secondary('ð')} ${fileName}${pages} - ${mimetype}`)
    },
    contactMessage: () => {
      const name = m.msg?.displayName || 'Contatto'
      const vcard = m.msg?.vcard ? ' (contatto)' : ''
      console.log(`${c.secondary('ð¤')} ${name}${vcard}`)
    },
    contactsArrayMessage: () => {
      const count = m.msg?.contacts?.length || '?'
      console.log(`${c.secondary('ð¨âð©âð§âð¦ ')} ${count} contatti`)
    },
    audioMessage: () => {
      const duration = formatDuration(m.msg?.seconds)
      const type = m.msg?.ptt ? 'Messaggio vocale' : 'Audio'
      const waveform = m.msg?.waveform ? ' ðµ' : ''
      console.log(`${c.secondary('ðµ ')} ${type}${duration ? ' - ' + duration : ''}${waveform}`)
    },
    videoMessage: () => {
      const duration = formatDuration(m.msg?.seconds)
      const gif = m.msg?.gifPlayback ? ' (GIF)' : ''
      console.log(`${c.secondary('ð¥')} Video${duration ? ' - ' + duration : ''}${gif}`)
    },
    imageMessage: () => {
      const width = m.msg?.width
      const height = m.msg?.height
      const dimensions = width && height ? ` (${width}x${height})` : ''
      console.log(`${c.secondary('ð¼ï¸')} Immagine${dimensions}`)
    },
    stickerMessage: () => {
      const animated = m.msg?.isAnimated ? ' (Animato)' : ''
      console.log(`${c.secondary('ð')} Sticker${animated}`)
    },
    reactionMessage: () => {
      const reaction = m.msg?.text || 'â¤ï¸'
      console.log(`${c.secondary('â§')} Reazione: ${reaction}`)
    },
    pollCreationMessage: () => {
      const question = m.msg?.name || 'Sondaggio'
      const options = m.msg?.options?.length || 0
      console.log(`${c.secondary('ð³ï¸')} ${question} (${options} opzioni)`)
    },
    productMessage: () => {
      ...
      console.log(`${c.secondary('ðï¸')} ${title}${price}`)
    },
    locationMessage: () => {
      ...
      console.log(`${c.secondary('ð')} ${name}${coords}`)
    },
    liveLocationMessage: () => {
      const duration = m.msg?.contextInfo?.expiration ? formatDuration(m.msg.contextInfo.expiration) : ''
      console.log(`${c.secondary('ð¡')} Posizione in tempo reale${duration ? ' - ' + duration : ''}`)
    },
  }
  ...
  if (m.mentionedJid) {
    for (const id of m.mentionedJid) {
      try {
        ...
        const name = await conn.getName(decodedId) || 'Utente'
