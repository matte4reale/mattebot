...
export async function handler(chatUpdate) {
    ...
    try {
        ...
        const isROwner = [conn.decodeJid(global.conn.user.id), ...(Array.isArray(global.owner) ? global.owner : []).map(([number]) => number)].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
