let handler = async (m, { conn, isAdmin }) => {
  ...
  if (m.text?.toLowerCase() === '.brum') {
    ...
    let loghi = [
      ...
      { url: 'https://www.freeiconspng.com/uploads/volkswagen-logo-png-transparent-7.png', marca: 'volkswagen' },
      { url: 'https://www.freepnglogos.com/uploads/lexus-logo-10.png', marca: 'lexus' },
      { url: 'https://www.freepnglogos.com/uploads/honda-logo-png/honda-logo-emblem-png-transparent-0.png', marca: 'honda' }
    ];

    let scelta = loghi[Math.floor(Math.random() * loghi.length)];
    let frasi = [
      '🚘 *INDOVINA LA MARCA DALLA FOTO DEL LOGO!*',
      '🚗 *Che marca è questa?*',
      '🏁 *Riconosci questo logo?*',
      '🔧 *Loghi d’auto: sai a chi appartiene?*'
    ];
    ...
    global.logoGame[m.chat] = {
      ...
      timeout: setTimeout(() => {
        if (global.logoGame?.[m.chat]) {
          conn.reply(m.chat, `⏰ Tempo scaduto! La risposta corretta era: *${scelta.marca}*`, m);
          delete global.logoGame[m.chat];
        }
      }, 60000)
    };
