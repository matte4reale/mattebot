...
const packPrices = { base: 100, imperium: 10000000, premium: 1000000000000 };
...
let leaderboardHandler = async (m, { conn }) => {
  ...
  if (users.length === 0) return m.reply('😢 Nessun collezionista trovato.');
...
// --- BUY PACKS HANDLER ---
let buyHandler = async (m, { conn, args }) => {
  const user = m.sender;
  ...
  const totalCost = packPrices[type] * quantity;
...
buyHandler.help = ['buy <tipo> <quantità>'];
buyHandler.tags = ['pokemon'];
buyHandler.command = /^buy$/i;

// --- OPEN PACKS HANDLER ---
let openPackHandler = async (m, { conn, args }) => {
  const user = m.sender;
  global.db.data.users[user] = global.db.data.users[user] || {};
  const data = global.db.data.users[user];
...
openPackHandler.help = ['apri <base|imperium|premium|darkness>'];
openPackHandler.tags = ['pokemon'];
openPackHandler.command = /^apri$/i;
